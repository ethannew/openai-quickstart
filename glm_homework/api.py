...
API_KEY: str = os.getenv("API_KEY", "replace your zhipu api key")
...
def get_characterglm_response(
    messages: TextMsgList, meta: CharacterMeta
    ...
    resp = requests.post(
        ...
        json=dict(model="charglm-3", meta=meta, prompt=messages, incremental=True),
    )
...
def get_characterglm_response_via_sdk(
    messages: TextMsgList, meta: CharacterMeta
) -> Generator[str, None, None]:
    ...
    response = zhipuai.model_api.sse_invoke(
        model="charglm-3", meta=meta, prompt=messages, incremental=True
    )
...
def generate_chat_scene_prompt(
    messages: TextMsgList, meta: CharacterMeta
) -> Generator[str, None, None]:
...
""".rstrip()
    ...
    if messages:
        instruction += "\n\n对话：" + "\n".join(
            (meta["bot_name"] if msg["role"] == "assistant" else meta["user_name"])
            + "："
            + msg["content"].strip()
            for msg in messages
        )
...
def generate_cogview_image(prompt: str) -> str:
    ...
    response = client.images.generations(
        model="cogview-3", prompt=prompt  # 填写需要调用的模型名称
    )
